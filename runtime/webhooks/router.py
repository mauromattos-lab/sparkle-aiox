...
async def asaas_onboarding_webhook(request: Request):
    ...
    """
    # Validar assinatura Asaas v3 (HMAC-SHA256) ou access-token simples
    webhook_secret = settings.asaas_webhook_secret
    if webhook_secret:
        # Asaas v3: header asaas-signature = sha256=<hmac_hex>
        body_bytes = await request.body()
        sig_header = request.headers.get("asaas-signature", "")
        secret_bytes = webhook_secret.encode()
        expected_sig = "sha256=" + hmac.new(secret_bytes, body_bytes, hashlib.sha256).hexdigest()
        if sig_header and not hmac.compare_digest(sig_header, expected_sig):
            logger.warning("[webhooks/asaas] assinatura HMAC inválida de %s", request.client.host if request.client else "unknown")
            return {"status": "ok", "processed": False, "reason": "invalid_signature"}
        elif not sig_header:
            logger.warning("[webhooks/asaas] asaas-signature ausente — modo permissivo (secret configurado)")
    else:
        # Fallback: validação simples por access-token
