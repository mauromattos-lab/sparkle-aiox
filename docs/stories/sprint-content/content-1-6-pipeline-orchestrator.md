...
- [ ] **AC2** — Pipeline avança: `briefed` → `image_generating` → `image_done` → (copy + video em paralelo) → `voice_generating` → `video_done` → `assembly_pending` → `assembly_done` → `pending_approval`
- [ ] **AC3** — Copy Specialist roda em paralelo com Video Generator (ambos podem usar `asyncio.gather` a partir de `image_done`)
- [ ] **AC4** — Estado `assembly_pending` só é atingido quando TODOS os assets estão prontos: `image_url`, `video_url`, `audio_url` (ou `audio_url=None` se sem narração), `caption`
